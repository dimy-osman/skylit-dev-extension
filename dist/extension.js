...
const postTypeConverter_1 = __webpack_require__(/*! ./postTypeConverter */ "./src/postTypeConverter.ts");
...
async function activate(context) {
    ...
    if (sites.length === 0) {
        ...
        vscode.window
            .showWarningMessage("Skylit.DEV plugin not detected. Install and activate the plugin in WordPress.", "Learn More")
            .then((selection) => {
...
function registerCommands(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("skylit.scanWorkspace", async () => {
        ...
        if (sites.length === 0) {
            vscode.window
                .showWarningMessage("No WordPress sites with Skylit.DEV plugin found. Check Output panel for details.", "View Output")
                .then((selection) => {
                if (selection ===
                    "View Output") {
                    debugLogger.show();
        ...
        vscode.window
            .showInformationMessage(`Found ${sites.length} WordPress site(s) with Skylit.DEV plugin!`, "Connect Now", "Setup Token")
            .then((selection) => {
            if (selection ===
                "Connect Now") {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("skylit.connect", async () => {
        ...
        if (sites.length === 0) {
            vscode.window
                .showErrorMessage("No WordPress sites found in workspace", "Scan Workspace")
                .then((selection) => {
                if (selection ===
                    "Scan Workspace") {
        ...
        if (sites.length > 1) {
            const choice = await vscode.window.showQuickPick(sites.map((s) => ({
                label: s.name,
    ...
    context.subscriptions.push(vscode.commands.registerCommand("skylit.convertPostType", async () => {
        if (!restClient) {
            vscode.window.showErrorMessage("Not connected to WordPress");
            return;
        }
        ...
        const explorerSelection = vscode.window.activeTextEditor?.document
            .uri.fsPath;
        ...
        if (!match) {
            vscode.window.showErrorMessage("Invalid folder name - must have _ID suffix");
            return;
        }
        ...
        // Confirm with user
        const confirm = await vscode.window.showWarningMessage(`Convert post ID ${postId} to "${targetType.label}"?`, { modal: true }, "Convert");
        if (confirm !== "Convert") {
            return;
        }
        try {
            debugLogger.log(`Converting post ${postId} to ${targetType.value}`);
            const response = await restClient.convertPostType(postId, targetType.value, folderName);
            if (response.success) {
                vscode.window.showInformationMessage(`✅ Post converted to ${targetType.label}!`);
            }
            else {
                vscode.window.showErrorMessage(`Failed to convert: ${response.message ||
                    "Unknown error"}`);
            }
        }
        catch (error) {
            vscode.window.showErrorMessage(`Failed to convert post type: ${error.message}`);
        }
    }));
    // Request WordPress ID command (create post for folder without valid ID)
    context.subscriptions.push(vscode.commands.registerCommand("skylit.requestPostId", async (uri) => {
        if (!restClient) {
            vscode.window.showErrorMessage("Not connected to WordPress");
            return;
        }
        ...
        if (uri) {
            ...
            const editor = vscode.window.activeTextEditor;
            if (editor) {
                folderPath = path.dirname(editor.document.uri
                    .fsPath);
            }
        }
        // Normalize path
        ...
        if (!folderPath) {
            vscode.window.showErrorMessage("No folder selected. Open a file or right-click on a folder.");
            return;
        }
        const folderName = path.basename(folderPath);
        // Determine post type from folder location
        const relativePath = folderPath.replace(currentDevPath.replace(/\\/g, "/") +
            "/", "");
        let postType;
        ...
        // Confirm with user
        const confirm = await vscode.window.showWarningMessage(`Create WordPress ${postType} from "${folderName}"?`, {
            modal: true,
        ...
        try {
            ...
            const response = await restClient.createPostFromFolder(relativePath, postType, false // Don't skip rename - let server do it
            );
            if (response.success) {
                const newId = response.post_id;
                const newFolder = response.new_folder ||
                    `${slug}_${newId}`;
                vscode.window.showInformationMessage(`✅ Created ${postType} "${response.title}" (ID: ${newId})`);
                debugLogger.log(`✅ Post created: ID=${newId}, folder renamed to ${newFolder}`);
                ...
                if (response.new_folder) {
                    ...
                    const newHtmlPath = path.join(currentDevPath, newBasePath, `${newFolderName}.html`);
                    // Try to open the new file
                    try {
                        const doc = await vscode.workspace.openTextDocument(newHtmlPath);
                        await vscode.window.showTextDocument(doc);
                    }
                    catch (e) {
                        // File might not exist yet, that's OK
                        debugLogger.log(`Note: Could not open ${newHtmlPath} - may need to refresh explorer`);
                    }
                }
            }
            else {
                vscode.window.showErrorMessage(`Failed to create post: ${response.error ||
                    response.message ||
                    "Unknown error"}`);
            }
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand("skylit.managePost", async (uri) => {
        if (!restClient) {
            vscode.window.showErrorMessage("Not connected to WordPress");
            return;
        }
        ...
        if (!idMatch) {
            vscode.window.showErrorMessage(`Folder "${folderName}" does not have a post ID suffix (e.g., my-page_123)`);
            return;
        }
        ...
        try {
            if (choice.value === statusOption) {
                ...
                if (response.success) {
                    ...
                    vscode.window.showInformationMessage(`✅ Post status changed to "${statusChoice.label.replace(/\$\(.*?\)\s*/, "")}"`);
                }
                else {
                    statusBar.showError("Update failed");
                    vscode.window.showErrorMessage(`Failed to update status: ${response.message ||
                        "Unknown error"}`);
                }
            }
                ...
                if (response.success) {
                    ...
                    vscode.window.showInformationMessage(`✅ Slug changed to "${newSlug}". Folder will be renamed automatically.`);
                    debugLogger.log(`✅ Slug updated: ${currentSlug} → ${newSlug}`);
                }
                else {
                    statusBar.showError("Update failed");
                    vscode.window.showErrorMessage(`Failed to update slug: ${response.message ||
                        "Unknown error"}`);
                }
            }
                ...
                if (response.success) {
                    ...
                    vscode.window.showInformationMessage(`✅ Title changed to "${newTitle}"`);
                    debugLogger.log(`✅ Title updated for post ${postId}`);
                }
                else {
                    statusBar.showError("Update failed");
                    vscode.window.showErrorMessage(`Failed to update title: ${response.message ||
                        "Unknown error"}`);
                }
            }
                ...
                if (response.success) {
                    ...
                    vscode.window.showInformationMessage(`✅ Post scheduled for ${dateStr}`);
                }
                else {
                    statusBar.showError("Scheduling failed");
                    vscode.window.showErrorMessage(`Failed to schedule post: ${response.message ||
                        "Unknown error"}`);
                }
            }
        }
            ...
            vscode.window.showErrorMessage(`Failed to update post: ${error.message}`);
            debugLogger.log(`❌ Manage post failed: ${error.message}`);
        }
    }));
    // Delete Post command (trash or permanently delete from WordPress)
    context.subscriptions.push(vscode.commands.registerCommand("skylit.deletePost", async (uri) => {
        if (!restClient) {
            vscode.window.showErrorMessage("Not connected to WordPress");
            return;
        }
        ...
        const cancelOption = "Cancel";
        const choice = await vscode.window.showWarningMessage(`Delete "${folderName}" from WordPress?`, {
            modal: true,
        ...
        }, trashOption, deleteOption, cancelOption);
        if (!choice || choice === cancelOption) {
            return;
        }
        const action = choice === trashOption
            ? "trash"
            : "delete";
        try {
            ...
            statusBar.showSyncing(`${action === "trash"
                ? "Trashing"
                : "Deleting"}...`);
            const response = await restClient.sendFolderAction(postId, action);
            if (response.success) {
                const actionVerb = action === "trash"
                    ? "moved to trash"
                    : "permanently deleted";
                ...
                try {
                    ...
                    debugLogger.log(`⚠️ Could not delete local folder: ${fsError.message}`);
                }
                // Delete metadata file for permanent deletes
                if (action === "delete") {
                    try {
                        const fs = await Promise.resolve().then(() => __importStar(__webpack_require__(/*! fs */ "fs")));
                        const metadataPath = path.join(currentDevPath, ".skylit", "metadata", `${postId}.json`);
                        ...
                        debugLogger.log(`⚠️ Could not delete metadata file: ${metaError.message}`);
                    }
                }
                statusBar.showSuccess(`Post ${actionVerb}`);
                vscode.window.showInformationMessage(`✅ Post ${postId} ${actionVerb} from WordPress`);
            }
            else {
                statusBar.showError("Action failed");
                vscode.window.showErrorMessage(`Failed to ${action} post: ${response.message ||
                    "Unknown error"}`);
            }
        }
        catch (error) {
        ...
        if (statusBar["connectionState"] ===
            ...
            const items = [
                ...
                },
                {
                    ...
                    description: "Manually scan workspace for WordPress + Skylit plugin",
                },
            ];
    ...
    try {
        ...
        if (!token) {
            ...
            if (isAutoConnect) {
                ...
                vscode.window
                    .showInformationMessage("Skylit.DEV: Auth token required", "Setup Token", "Dismiss")
                    .then((selection) => {
                    if (selection ===
                        "Setup Token") {
                        vscode.commands.executeCommand("skylit.setupToken");
        ...
        if (!isValid) {
            ...
            if (isAutoConnect) {
                ...
                vscode.window
                    .showWarningMessage("Skylit.DEV: Auth token is invalid or expired", "Setup New Token", "Dismiss")
                    .then((selection) => {
                    if (selection ===
                        "Setup New Token") {
                        vscode.commands.executeCommand("skylit.setupToken");
            ...
            vscode.window
                .showErrorMessage("Invalid auth token. Please generate a new one in WordPress Admin → Skylit.DEV → Dev Sync", "Setup Token")
                .then((selection) => {
        ...
        postTypeConverter = new postTypeConverter_1.PostTypeConverter(restClient, status.dev_path, debugLogger);
        ...
        statusCheckInterval = setInterval(async () => {
            ...
            try {
                ...
                if (updatedStatus.dev_path !== currentDevPath) {
                    ...
                    postTypeConverter =
                        new postTypeConverter_1.PostTypeConverter(restClient, updatedStatus.dev_path, debugLogger);
                    postTypeConverter.startWatching();
    ...
    try {
        ...
        if (error.message &&
            ...
            if (consecutiveErrors > 1) {
                ...
                // Add jitter (±25%)
                const jitter = jumpPollIntervalMs *
                    0.25 *
                    (Math.random() - 0.5);
                jumpPollIntervalMs = Math.round(jumpPollIntervalMs + jitter);
                debugLogger.warn(`⚠️ Jump polling error (${consecutiveErrors} consecutive): ${error.message}. ` +
    ...
    async restoreFoldingState(filePath, unchangedBlocks) {
        ...
        for (const foldLine of savedFolds) {
            ...
            const isInUnchangedBlock = unchangedBlocks.some((block) => {
                ...
                return (foldLine >= block.startLine &&
...
class FileWatcher {
    constructor(devFolder, restClient, statusBar, debugLogger) {
        ...
        this.recentlyRenamedFolders = new Map(); // Track old→new renames to prevent duplicates
        ...
        this.skipDeletePromptForPosts = new Set(); // Track posts being deleted via command to skip FileWatcher prompt
    ...
    async start() {
        ...
        this.vscodeTrashWatcher =
            vscode.workspace.createFileSystemWatcher(trashPattern, false, true, false);
     ...
     */
    async checkAndSyncHtmlMetadata(postId, html, postFolder) {
        ...
        if (htmlMetadata.id && htmlMetadata.id !== postId) {
            this.debugLogger.log(`⚠️ HTML metadata ID (${htmlMetadata.id}) doesn't match folder ID (${postId})`);
            return false;
        }
        ...
        if (htmlMetadata.slug && htmlMetadata.slug !== cached.slug) {
            ...
            hasChanges = true;
            this.debugLogger.log(`📝 [HTML Metadata] Slug changed: ${cached.slug} → ${htmlMetadata.slug}`);
        }
        if (htmlMetadata.title && htmlMetadata.title !== cached.title) {
            changes.title = htmlMetadata.title;
            hasChanges = true;
            this.debugLogger.log(`📝 [HTML Metadata] Title changed: ${cached.title} → ${htmlMetadata.title}`);
        }
        if (htmlMetadata.status &&
            ...
            if (validStatuses.includes(htmlMetadata.status)) {
                ...
                hasChanges = true;
                this.debugLogger.log(`📝 [HTML Metadata] Status changed: ${cached.status} → ${htmlMetadata.status}`);
            }
        }
        if (!hasChanges) {
            return false;
        ...
        if (changes.slug && cached.slug) {
            ...
            await this.renamePostFilesUniversal(postId, cached.slug, changes.slug, postType, postFolder);
        }
        // Sync changes to WordPress
        this.statusBar.showSyncing("Syncing metadata...");
        try {
            ...
            if (response.success) {
                ...
                this.debugLogger.log(`⚠️ [HTML Metadata] Sync failed: ${response.error}`);
                return false;
            }
        }
        catch (error) {
     ...
     * Uses VS Code's workspace.fs API for SSH compatibility
     */
    async renamePostFilesUniversal(postId, oldSlug, newSlug, postType, currentFolderPath) {
        this.debugLogger.log(`📂 Renaming files for ${postType} ${postId}: ${oldSlug} → ${newSlug}`);
        try {
     ...
     */
    async updateHtmlMetadataSlug(htmlPath, newSlug) {
        try {
            ...
            this.debugLogger.log(`⚠️ Could not update HTML metadata: ${error.message}`);
        }
    }
    /**
     ...
     */
    async promptDeleteAction(postId, folderName) {
        ...
        if (this.skipDeletePromptForPosts.has(postId)) {
            ...
            this.debugLogger.log(`⏭️ Skipping delete prompt for post ${postId} (already handled via command)`);
            return;
        }
        const trashOption = "Move to Trash";
        const deleteOption = "Delete Permanently";
        const keepOption = "Keep in WordPress";
        const choice = await vscode.window.showWarningMessage(`Folder "${folderName}" was deleted`, {
            modal: true,
        ...
        }, trashOption, deleteOption, keepOption);
        ...
        try {
            ...
            this.statusBar.showSyncing(`${action === "trash"
                ? "Trashing"
                : "Deleting"} post...`);
            ...
            if (response.success) {
                ...
                this.statusBar.showSuccess(`Post ${actionVerb}`);
                vscode.window.showInformationMessage(`✅ Post ${postId} ${actionVerb} from WordPress`);
    ...
    deleteMetadataFile(postId) {
        try {
            ...
            this.debugLogger.log(`⚠️ Could not delete metadata file: ${error.message}`);
        }
    }
    /**
     ...
     */
    async findExistingFolderWithId(folderName, postTypeFolder) {
        try {
    ...
    async createPostFromNewFolder(folderPath, postTypeFolder, folderName, relativePath) {
        ...
        try {
            ...
            if (response.success && response.post_id) {
                ...
                this.statusBar.showError(response.error ||
                    ...
                    for (const [fileName, fileType,] of files) {
                        if (fileType ===
                            ...
                            if (newFileName !==
                                ...
                                const oldFilePath = posixJoin(folderPath, fileName);
                                const newFilePath = posixJoin(folderPath, newFileName);
                                await vscode.workspace.fs.rename(pathToUri(oldFilePath), pathToUri(newFilePath), {
                                    overwrite: true,
                                });
                            }
                        }
    ...
    async writePostCreationNotification(response, postType) {
        ...
        try {
            ...
            const slug = response.slug ||
                (response.new_folder
                    ? response.new_folder.replace(/_\d+$/, "")
                    : "");
    ...
    async scanForNewFolders(postTypesPath) {
        ...
        try {
            ...
            const postTypeDirs = postTypeDirEntries.filter(([name, type]) => type === vscode.FileType.Directory &&
                !name.startsWith(".") &&
                name !== "_trash");
            ...
            for (const [postTypeDirName] of postTypeDirs) {
                ...
                const contentFolders = contentFolderEntries.filter(([name, type]) => type ===
                    vscode.FileType
                        .Directory &&
                    !name.startsWith(".") &&
                    name !== "_trash" &&
                ...
                for (const [folderName] of contentFolders) {
                    ...
                    const files = await vsReadDir(folderPath);
                    const hasHtml = files.some(([name, type]) => type ===
                        vscode.FileType
                            .File &&
                        name.endsWith(".html"));
                    if (!hasHtml) {
    ...
    async handleThemeAssetChange(filePath) {
        ...
        if (timeSinceLastSync < this.themeSyncCooldownMs) {
            this.debugLogger.log(`⏸️ Skipping theme→dev sync (cooldown: ${Math.round((this.themeSyncCooldownMs -
                timeSinceLastSync) /
                1000)}s remaining)`);
            return;
        }
    ...
    handlePotentialTrashAction(dirPath, eventType) {
        ...
        if (eventType === "add" && isInTrash) {
            ...
            for (const [oldName, renameInfo,] of this.recentlyRenamedFolders.entries()) {
                if (renameInfo.newFolder === folderName &&
    ...
    async debounceFolderAction(postId, action) {
        ...
        if (timeSinceLastAction < this.folderActionCooldownMs) {
            this.debugLogger.log(`⏸️ Skipping folder action (cooldown: ${Math.round((this.folderActionCooldownMs -
                timeSinceLastAction) /
                1000)}s remaining)`);
            return;
        }
    ...
    async handleThemeFileChange(filePath) {
        ...
        if (timeSinceLastSync < this.syncCooldownMs) {
            this.debugLogger.log(`⏸️ Skipping dev→theme sync (cooldown: ${Math.round((this.syncCooldownMs -
                timeSinceLastSync) /
                1000)}s remaining)`);
            return;
        }
        ...
        try {
            ...
            if (this.themePath) {
                const themeFilePath = this.themePath.replace(/\\/g, "/") +
                    "/" +
                    relativePath;
    ...
    async handleIncludeChange(filePath) {
        ...
        const timeSinceLastSync = now - lastSync;
        if (timeSinceLastSync < this.syncCooldownMs) {
            this.debugLogger.log(`⏸️ Skipping dev→theme PHP sync (cooldown: ${Math.round((this.syncCooldownMs -
                timeSinceLastSync) /
                1000)}s remaining)`);
            return;
        }
    ...
    async handleThemeStructureChange(filePath) {
        ...
        const timeSinceLastSync = now - lastSync;
        if (timeSinceLastSync < this.syncCooldownMs) {
            this.debugLogger.log(`⏸️ Skipping theme structure sync (cooldown: ${Math.round((this.syncCooldownMs -
                timeSinceLastSync) /
                1000)}s remaining)`);
            return;
        }
    ...
    async syncFile(filePath) {
        try {
            ...
            const timeSinceLastSync = now - lastSync;
            if (timeSinceLastSync < this.syncCooldownMs) {
                this.debugLogger.log(`⏸️ Skipping sync (cooldown: ${Math.round((this.syncCooldownMs -
                    timeSinceLastSync) /
                    1000)}s remaining)`);
                return;
            }
            ...
            try {
                ...
                if (checkResult.skip_import) {
                    this.debugLogger.log(`⏭️ Skipping sync (recent export - circular sync prevention)`);
            ...
            if (html) {
                const metadataChanged = await this.checkAndSyncHtmlMetadata(postId, html, postFolder);
                if (metadataChanged) {
                    ...
                    this.debugLogger.log(`📝 Metadata changed - skipping content sync (will re-sync after rename)`);
                    return;
                }
            }
            ...
            if (response.success) {
                ...
                const config = vscode.workspace.getConfiguration("skylit");
                if (config.get("showNotifications", true)) {
                    vscode.window.showInformationMessage(`✅ Synced: ${fileName} (${response.blocks_updated ||
                        0} blocks)`);
                }
            }
        }
    ...
    async processCursorPosition(editor) {
        try {
            ...
            this.debugLogger.log(`🎯 [Cursor] Processing: line ${cursorLine} in ${filePath
                .split("/")
                .pop()}`);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.PostTypeConverter = void 0;
const vscode = __importStar(__webpack_require__(/*! vscode */ "vscode"));
const path = __importStar(__webpack_require__(/*! path */ "path"));
/**
 * Post Type Converter
...
class PostTypeConverter {
    constructor(restClient, devFolderPath, logger) {
        this.disposables = [];
    ...
    startWatching() {
        ...
        const pattern = new vscode.RelativePattern(this.devFolderPath, '**/*');
     ...
     */
    async handleFolderMove(oldPath, newPath) {
        try {
     ...
     */
    async getPostTitle(folderPath) {
        try {
            // Find HTML files in folder
            const files = await vscode.workspace.findFiles(new vscode.RelativePattern(folderPath, '*.html'), null, 1);
            if (files.length === 0) {
     ...
     */
    async showConversionPrompt(postId, postTitle, sourceType, targetType, oldPath, newPath) {
        const sourceLabel = this.getTypeLabel(sourceType);
        const targetLabel = this.getTypeLabel(targetType);
        const action = await vscode.window.showWarningMessage(`Convert "${postTitle}" from ${sourceLabel} to ${targetLabel}?`, {
            modal: true,
            detail: `You moved this folder from ${sourceLabel} to ${targetLabel}. Do you want to update the post type in WordPress to match?`
        }, 'Convert in WordPress', 'Undo Move', 'Ignore');
     ...
     */
    async performConversion(postId, postTitle, targetType, newPath) {
        try {
            await vscode.window.withProgress({
                ...
                if (result.success) {
                    ...
                    vscode.window.showInformationMessage(`✓ "${postTitle}" converted successfully to ${this.getTypeLabel(targetType)}`);
     ...
     * Undo folder move
     */
    async undoMove(oldPath, newPath, postTitle) {
        try {
            ...
            // Move folder back
            await vscode.workspace.fs.rename(vscode.Uri.file(newPath), vscode.Uri.file(oldPath), { overwrite: false });
            vscode.window.showInformationMessage(`✓ "${postTitle}" moved back to original location`);
            this.logger.log('Post Type Converter: Move undone successfully');
        }
        catch (error) {
            vscode.window.showErrorMessage(`Failed to undo move: ${error.message}`);
     ...
     */
    async updateLocalMetadata(folderPath, conversionResult) {
        try {
            // Find HTML file
            const files = await vscode.workspace.findFiles(new vscode.RelativePattern(folderPath, '*.html'), null, 1);
            if (files.length === 0) {
            ...
            if (text.includes('WordPress Sync Metadata')) {
                text = text.replace(/<!--\s*WordPress Sync Metadata[\s\S]*?-->/, metadataComment);
            }
            else {
                text = metadataComment + '\n' + text;
            }
            // Write back
            await vscode.workspace.fs.writeFile(files[0], Buffer.from(text, 'utf8'));
...
exports.PostTypeConverter = PostTypeConverter;
...
class RestClient {
    ...
    async syncFile(postId, html, css) {
        ...
        try {
            ...
            if (response.data.success) {
                this.debugLogger.log(`✅ Synced! ${response.data.blocks_updated ||
                    0} blocks updated`);
            }
            return response.data;
        }
    ...
    async syncAssets(direction = "to_theme") {
        const directionLabel = direction === "to_theme"
            ? "dev → theme"
            : "theme → dev";
    ...
    async importNewFiles() {
        ...
        try {
            ...
            if (response.data.success) {
                this.debugLogger.log(`✅ Import complete: ${response.data.imported
                    ?.length || 0} imported, ` +
     ...
     * Convert post type
     */
    async convertPostType(postId, targetType, folderName) {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                this.debugLogger.log(`❌ Conversion failed: ${response.data.error ||
                    "Unknown error"}`);
            }
            return response.data;
        }
    ...
    async updatePostMeta(postId, metadata) {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                this.debugLogger.log(`❌ Update failed: ${response.data.error ||
                    "Unknown error"}`);
            }
            return response.data;
        }
