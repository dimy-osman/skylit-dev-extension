...
async function activate(context) {
    ...
    // Initialize managers
    workspaceManager = new workspaceManager_1.WorkspaceManager(outputChannel);
    authManager = new authManager_1.AuthManager(context, outputChannel);
    statusBar = new statusBar_1.StatusBar(outputChannel);
    protocolHandler = new protocolHandler_1.ProtocolHandler(outputChannel);
...
async function connectToWordPress(site, context, isAutoConnect = false) {
    ...
    try {
        ...
        // Create REST client
        restClient = new restClient_1.RestClient(site.siteUrl, token, outputChannel);
        // Validate token
        ...
        fileWatcher = new fileWatcher_1.FileWatcher(status.dev_path, restClient, statusBar, outputChannel);
        ...
        statusCheckInterval = setInterval(async () => {
            ...
            try {
                ...
                if (updatedStatus.dev_path !== currentDevPath) {
                    ...
                    fileWatcher = new fileWatcher_1.FileWatcher(updatedStatus.dev_path, restClient, statusBar, outputChannel);
                    await fileWatcher.start();
...
class FileWatcher {
    constructor(devFolder, restClient, statusBar, outputChannel) {
        this.watcher = null;
    ...
    async start() {
        ...
        this.trashWatcher = chokidar.watch(this.devFolder, {
            ignored: [
                /(^|[\/\\])\../, // Ignore dotfiles
                '**/node_modules/**',
                '**/.git/**',
    ...
    async createPostFromNewFolder(folderPath, postTypeFolder, folderName, relativePath) {
        ...
        try {
            ...
            if (response.success && response.post_id) {
                ...
                this.outputChannel.appendLine(`✅ Created ${postType} "${response.title}" (ID: ${response.post_id})`);
    ...
    async handleThemeAssetChange(filePath) {
        ...
        if (timeSinceLastSync < this.themeSyncCooldownMs) {
            this.outputChannel.appendLine(`⏸️ Skipping theme→dev sync (cooldown: ${Math.round((this.themeSyncCooldownMs - timeSinceLastSync) / 1000)}s remaining)`);
            return;
        }
     ...
     */
    debounceFolderAction(postId, action) {
        ...
        if (timeSinceLastAction < this.folderActionCooldownMs) {
            this.outputChannel.appendLine(`⏸️ Skipping folder action (cooldown: ${Math.round((this.folderActionCooldownMs - timeSinceLastAction) / 1000)}s remaining)`);
            return;
        }
    ...
    async handleThemeFileChange(filePath) {
        ...
        if (timeSinceLastSync < this.syncCooldownMs) {
            this.outputChannel.appendLine(`⏸️ Skipping dev→theme sync (cooldown: ${Math.round((this.syncCooldownMs - timeSinceLastSync) / 1000)}s remaining)`);
            return;
        }
        // Determine file type for logging
    ...
    async handleIncludeChange(filePath) {
        ...
        const timeSinceLastSync = now - lastSync;
        if (timeSinceLastSync < this.syncCooldownMs) {
            this.outputChannel.appendLine(`⏸️ Skipping dev→theme PHP sync (cooldown: ${Math.round((this.syncCooldownMs - timeSinceLastSync) / 1000)}s remaining)`);
            return;
        }
    ...
    async handleThemeStructureChange(filePath) {
        ...
        const timeSinceLastSync = now - lastSync;
        if (timeSinceLastSync < this.syncCooldownMs) {
            this.outputChannel.appendLine(`⏸️ Skipping theme structure sync (cooldown: ${Math.round((this.syncCooldownMs - timeSinceLastSync) / 1000)}s remaining)`);
            return;
        }
        // Determine file type for logging
        let fileType = 'theme file';
    ...
    async syncFile(filePath) {
        try {
            ...
            const timeSinceLastSync = now - lastSync;
            if (timeSinceLastSync < this.syncCooldownMs) {
                this.outputChannel.appendLine(`⏸️ Skipping sync (cooldown: ${Math.round((this.syncCooldownMs - timeSinceLastSync) / 1000)}s remaining)`);
                return;
            }
            ...
            try {
                ...
                if (checkResult.skip_import) {
                    this.outputChannel.appendLine(`⏭️ Skipping sync (recent export - circular sync prevention)`);
                    return;
                }
            }
            catch (error) {
...
const vscode = __importStar(__webpack_require__(/*! vscode */ "vscode"));
class RestClient {
    constructor(siteUrl, token, outputChannel) {
    ...
    async validateToken() {
        try {
            ...
            if (response.data.valid) {
                this.outputChannel.appendLine(`✅ Token valid for user: ${response.data.user?.name || 'Unknown'}`);
                return true;
            }
            return false;
        }
    ...
    async syncFile(postId, html, css) {
        ...
        try {
            ...
            if (response.data.success) {
                this.outputChannel.appendLine(`✅ Synced! ${response.data.blocks_updated || 0} blocks updated`);
            }
            return response.data;
        }
        catch (error) {
    ...
    async importNewFiles() {
        ...
        try {
            ...
            if (response.data.success) {
                this.outputChannel.appendLine(`✅ Import complete: ${response.data.imported?.length || 0} imported, ` +
     ...
     */
    async createPostFromFolder(folderPath, postType) {
        ...
        try {
            ...
            if (response.data.success) {
                this.outputChannel.appendLine(`✅ Created: ${response.data.title} (ID: ${response.data.post_id})`);
            }
            return response.data;
        }
        catch (error) {
    ...
    async updatePostSlug(postId, newSlug) {
        ...
        try {
            ...
            if (response.data.success) {
                this.outputChannel.appendLine(`✅ Slug updated: ${response.data.old_slug} → ${response.data.new_slug}`);
            }
            return response.data;
        }
        catch (error) {
    ...
    async updateFromMetadata(postId, metadata) {
        ...
        try {
            ...
            if (response.data.success) {
                this.outputChannel.appendLine(`✅ Metadata synced: ${response.data.message}`);
            }
            return response.data;
        }
        catch (error) {
