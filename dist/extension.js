...
class FileWatcher {
    ...
    async handleThemeAssetChange(filePath) {
        ...
        if (timeSinceLastSync < this.themeSyncCooldownMs) {
            this.debugLogger.log(`⏸️ Skipping theme→dev sync (cooldown: ${Math.round((this.themeSyncCooldownMs - timeSinceLastSync) / 1000)}s remaining)`);
            return;
        }
    ...
    async debounceFolderAction(postId, action) {
        ...
        if (timeSinceLastAction < this.folderActionCooldownMs) {
            this.debugLogger.log(`⏸️ Skipping folder action (cooldown: ${Math.round((this.folderActionCooldownMs - timeSinceLastAction) / 1000)}s remaining)`);
            return;
        }
    ...
    async handleThemeFileChange(filePath) {
        ...
        if (timeSinceLastSync < this.syncCooldownMs) {
            this.debugLogger.log(`⏸️ Skipping dev→theme sync (cooldown: ${Math.round((this.syncCooldownMs - timeSinceLastSync) / 1000)}s remaining)`);
            return;
        }
        // Determine file type for logging
    ...
    async handleIncludeChange(filePath) {
        ...
        const timeSinceLastSync = now - lastSync;
        if (timeSinceLastSync < this.syncCooldownMs) {
            this.debugLogger.log(`⏸️ Skipping dev→theme PHP sync (cooldown: ${Math.round((this.syncCooldownMs - timeSinceLastSync) / 1000)}s remaining)`);
            return;
        }
    ...
    async handleThemeStructureChange(filePath) {
        ...
        const timeSinceLastSync = now - lastSync;
        if (timeSinceLastSync < this.syncCooldownMs) {
            this.debugLogger.log(`⏸️ Skipping theme structure sync (cooldown: ${Math.round((this.syncCooldownMs - timeSinceLastSync) / 1000)}s remaining)`);
            return;
        }
        // Determine file type for logging
        let fileType = 'theme file';
    ...
    async syncFile(filePath) {
        try {
            ...
            const timeSinceLastSync = now - lastSync;
            if (timeSinceLastSync < this.syncCooldownMs) {
                this.debugLogger.log(`⏸️ Skipping sync (cooldown: ${Math.round((this.syncCooldownMs - timeSinceLastSync) / 1000)}s remaining)`);
                return;
            }
