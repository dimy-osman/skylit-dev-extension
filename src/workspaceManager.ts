...
export class WorkspaceManager {
	...
	constructor(debugLogger: DebugLogger) {
		...
		if (workspaceFolders.length > 0) {
			...
			this.isRemote =
				firstFolder.uri.scheme === "vscode-remote";
	...
	async detectWordPressSites(): Promise<WordPressSite[]> {
		...
		this.debugLogger.log(
			"🔍 Scanning workspace for WordPress sites..."
		);
		...
		for (const folder of workspaceFolders) {
			...
			const site = await this.detectWordPressInFolder(
				folderPath
			);
			if (site) {
				sites.push(site);
	 ...
	 */
	private async detectWordPressInFolder(
		folderPath: string
	): Promise<WordPressSite | null> {
		this.debugLogger.log(
			`   📁 Starting search from: ${folderPath}`
		);
		...
		if (!wpRoot) {
			this.debugLogger.log(
				`   🔍 Searching downward into subdirectories...`
			);
			wpRoot = await this.findWordPressInSubdirectories(
				folderPath
			);
		}
		...
		if (!isSkylitInstalled) {
			this.debugLogger.log(
				`   ⚠️ Skylit.DEV plugin not found or not activated`
			);
			this.debugLogger.log(
				`   ℹ️ Please install and activate the Skylit.DEV plugin in WordPress`
			);
			return null;
		}

		this.debugLogger.log(
			`   ✅ Skylit.DEV plugin detected and active!`
		);
		...
		try {
			...
			// Check if user has manually configured site URL in settings (overrides .skylit/site.json)
			const vscodeConfig =
				vscode.workspace.getConfiguration("skylit");
			const manualSiteUrl =
				vscodeConfig.get<string>("siteUrl");

			if (manualSiteUrl && manualSiteUrl.trim() !== "") {
				config.siteUrl = manualSiteUrl
					.trim()
					.replace(/\/$/, "");
				this.debugLogger.log(
					`   ✅ Using site URL from VS Code settings (skylit.siteUrl): ${config.siteUrl}`
				);
			}
			...
			this.debugLogger.log(
				`   ⚠️ Error parsing wp-config.php: ${error.message}`
			);
			return null;
		}
	}
		...
		for (const pluginPath of pluginPaths) {
			if (await this.fileExists(pluginPath)) {
				...
				try {
					...
					if (
						content.includes(
							"Plugin Name: Skylit.DEV"
						) ||
						content.includes(
							"Skylit Dev UI"
						) ||
						content.includes(
							"SKYLIT_DEV_UI_VERSION"
						)
					) {
						...
						if (versionMatch) {
							this.debugLogger.log(
								`   ℹ️ Plugin version: ${versionMatch[1]}`
							);
						}
					...
					this.debugLogger.log(
						`   ⚠️ Could not read plugin file: ${error.message}`
					);
				}
			}
		}
	 ...
	 */
	private async findWordPressRoot(
		startPath: string
	): Promise<string | null> {
	 ...
	 * Returns a candidate URL from wp-config.php. The caller should use
	 * RestClient.discover() to get the canonical URL from WordPress.
	 */
	private async parseWpConfig(configPath: string): Promise<WpConfig> {
		...
		// Extract site URL from wp-config.php (may be localhost/placeholder)
		let siteUrl =
			this.extractDefine(content, "WP_HOME") ||
		...
		if (!siteUrl) {
			...
			this.debugLogger.log(
				`   ⚠️ No WP_HOME/WP_SITEURL in wp-config.php, using ${siteUrl}`
			);
		}

		// Find dev folder (for file watching - canonical path comes from discovery)
	...
	private async findDevFolder(wpRoot: string): Promise<string> {
		...
		this.debugLogger.log(
			`   ⚠️ Dev folder not found, defaulting to wp-content`
		);
		return wpContent;
	}
